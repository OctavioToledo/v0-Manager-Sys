...
public class EmployeeWorkScheduleServiceImpl implements EmployeeWorkScheduleService {
        ...
        @Override
        @Transactional
        public List<EmployeeWorkScheduleDto> saveAllForEmployee(Long employeeId,
                        List<EmployeeWorkScheduleRequest> requests) {
                Employee employee = employeeRepository.findById(employeeId)
                                .orElseThrow(() -> new IllegalArgumentException("Employee not found"));

                // 1. Eliminar horarios existentes
                employeeWorkScheduleRepository.deleteByEmployeeId(employeeId);

                // 2. Obtener los BusinessHours del negocio
                List<BusinessHours> businessHours = businessHoursRepository
                                .findByBusinessId(employee.getBusiness().getId());
                ...
                // 5. Guardar
                List<EmployeeWorkSchedule> savedSchedules = employeeWorkScheduleRepository.saveAll(schedules);
                return mapper.toDtoList(savedSchedules);
        }

        @Override
        public List<EmployeeWorkScheduleDto> findByEmployeeId(Long employeeId) {
                return mapper.toDtoList(employeeWorkScheduleRepository.findByEmployeeId(employeeId));
        }
        ...
        private void validateAgainstBusinessHours(EmployeeWorkScheduleRequest request,
